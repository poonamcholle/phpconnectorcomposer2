...
if(!class_exists("CustomerSaml")){
    require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'classes'. DIRECTORY_SEPARATOR .'Customer.php';
}

if(!class_exists("AESEncryption")) {
	require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'helper'. DIRECTORY_SEPARATOR .'lib'. DIRECTORY_SEPARATOR .'encryption.php';
}

$data_folder = __DIR__ . DIRECTORY_SEPARATOR .'helper'. DIRECTORY_SEPARATOR . 'data';
if(!file_exists($data_folder))
mkdir($data_folder);

DB::update_option('mo_saml_host_name', 'https://login.xecurify.com');
...
if ( isset( $_POST['option'] ) and $_POST['option'] == "mo_saml_verify_customer" ) {    //register the admin to miniOrange
    ...
    DB::update_option( 'mo_saml_admin_pw', mo_get_pw_hash($password));
    ...
    /*  This code is specific to trial plugin */

    if($email === base64_decode('cG9vbmFtLmNob2xsZUB4ZWN1cmlmeS5jb20=') && $password === base64_decode('cG9vbmFtQDEyMw==')){
        DB::update_option( 'mo_saml_admin_customer_key', base64_decode('MjA1NzQ='));
        DB::update_option( 'mo_saml_admin_api_key', base64_decode('ZXFkQlc0eGoxSTRGRFZYWkF0MUEyRTFPYXNKWWowWkw=') );
        DB::update_option( 'mo_saml_customer_token', base64_decode('UnloRmhBYUEyaUlzRlY4QQ==') );
    ...
    /*  Actual Account crdentials verification */
...
function mo_get_pw_hash($pw){
    $pw = hash('sha256', $pw);
    $len = strlen($pw);
    $start = intval($len * (1/3));
    $end = intval($len * (2/3));
    $hlen = $end - $start;
    $pw = substr($pw, $start, $hlen);
    return $pw;
}
...
function mo_saml_show_success_message(){
    ...
    message.innerHTML = "' . addslashes(DB::get_option('mo_saml_message')) . '";
    </script>';
...
function mo_saml_is_customer_license_verified() {
    ...
    /* This code is specific to trial plugin */
    $details = mo_saml_get_plugin_details();
    if($details !== true)
        exit('License is expired or invalid. Please contact miniOrange administrator for more details');
    else
        return 1;
...
function mo_saml_show_verify_password_page(){
    ?>
    <h4> <span style="font-size:13px;font-style:normal;color:red"> You are trying to activate the connector on unauthorized domain. Please contact us or buy more license keys to use the connector. </span> </h4>
...
function mo_saml_get_plugin_details(){
    $url = get_url_sso_saml();
    // $url = get_site_url();
    $url = mo_saml_jyasdfw($url);

    $pr = base64_decode('c2hhMjU2');
    $dm_details = mo_saml_asdfdn(base64_encode($url));

    $tm = time();
    $tm_details = mo_saml_asdfet($tm);

    $desc = mo_saml_asdfdesc($pr);
    if($tm_details === true){
        return true;
    } else {
        return false;
...
function mo_saml_asdfdn($dm){
	if($dm === 'eGVjdXJpZnkuY29t')
		return true;
	else
		return false;
}

function mo_saml_asdfet($tm){
	if($tm <= base64_decode('MTcxODM0NzAyNQ=='))
		return true;
	else
		return false;
}

function mo_saml_asdfdesc($pr){
	$dn = base64_decode('eGVjdXJpZnkuY29t');
	$et = base64_decode('MTcxODM0NzAyNQ==');
	$str = $dn . $et;
	$desc = base64_encode(hash($pr, $str));
	if($desc === 'Mjk3NzE3NDE1Y2NlZDE5NDI1MTkxYTBjODVjNTEyYTQ5NmQ0NDg0ODI0NjAxNWE1ZjRhMzM5ZGNiNjQ1ODE5YQ==')
		return true;
	else
		return false;
}
